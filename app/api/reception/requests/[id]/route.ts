...
import { ReceptionPriority, ReceptionStatus, updateReceptionRequest } from "../../../../../lib/reception-store";
...
export async function PATCH(request: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  if (typeof body.status === "string" && statuses.has(body.status as ReceptionStatus)) patch.status = body.status;
  if (typeof body.priority === "string" && priorities.has(body.priority as ReceptionPriority)) patch.priority = body.priority;
